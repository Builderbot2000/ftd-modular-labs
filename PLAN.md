# FTD Modular Labs — Framework Scaffold Plan (Uno Platform)
...
`ftd-modular-labs` will become a desktop app that answers *From The Depths* design questions by
...
FtdModularLabs.sln
src/
  FtdModularLabs.Core/            net9.0 class library — NO UI deps (the framework contracts)
  FtdModularLabs.Modules.Demo/    net9.0 — a trivial demo ICalculationModule
  FtdModularLabs.App/             Uno single-project app (host shell); references Core + Demo
tests/
  FtdModularLabs.Core.Tests/      xUnit — tests Core + Demo module
```

Rationale: `FtdModularLabs.Core` stays pure .NET so the calc engine is UI-framework-agnostic,
fast to unit-test, and (later) drops unchanged into an Uno WASM head. UI project contains **no
calculation logic**.

## FtdModularLabs.Core — the framework contracts (the heart of this pass)
...
## FtdModularLabs.Modules.Demo — proves the plumbing
...
## FtdModularLabs.App — Uno shell (host)
...
1. `dotnet build FtdModularLabs.sln` — solution compiles.
...
3. `dotnet run --project src/FtdModularLabs.App` (Skia.Desktop head) — app launches; the Demo module
...
- Porting the real ApsCalc optimizer (MIT, https://github.com/AoKishuba/ApsCalcUI) into
  `FtdModularLabs.Modules.Aps`. Its calc core (`Shell.cs`, `ShellCalc.cs`, `Module.cs`, `Layer.cs`,
